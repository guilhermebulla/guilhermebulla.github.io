/* ===== REPERTORIO.JS v2.0 — Tabela + Autocomplete + Star + Décadas ===== */
...
let songStates = {}; // 'starred' | 'neutral'
...
function renderArtistList(artists) {
    ...
    artistList.innerHTML = artists.map(artist => {
        ...
        return '<div class="artist-item">' +
            ...
            '<button class="artist-star-btn ' + starClass + '" data-artist="' + escapeHtml(artist) + '" title="Estrelar todas as músicas">★</button>' +
        '</div>';
    }).join('');
}
...
function showAutocomplete(term) {
    ...
    // Letter shortcut: 1 char
    if (term.length === 1 && /[a-zà-ú]/i.test(term)) {
        const letter = term.toUpperCase();
        const letterNorm = normalizeText(letter);
        const artistsWithLetter = uniqueArtists.filter(a =>
            normalizeText(a.charAt(0)) === letterNorm
        );
        if (artistsWithLetter.length > 0) {
            html += '<div class="autocomplete-section">';
            html += '<div class="autocomplete-section-label">Atalho</div>';
            html += '<div class="autocomplete-item" data-type="letter" data-letter="' + escapeHtml(letter) + '">';
            html += '🎤 Artistas com a letra "' + escapeHtml(letter) + '" (' + artistsWithLetter.length + ')</div>';
            html += '</div>';
        }
    }

    // If in letter mode, show artists of that letter
    if (autocompleteLetterMode && autocompleteLetterMode.length === 1) {
        const letterNorm = normalizeText(autocompleteLetterMode);
        const artists = uniqueArtists.filter(a =>
            normalizeText(a.charAt(0)) === letterNorm
        ).slice(0, 20);
        html += '<div class="autocomplete-section">';
        html += '<div class="autocomplete-section-label">Artistas com "' + escapeHtml(autocompleteLetterMode.toUpperCase()) + '"</div>';
        html += '<div class="autocomplete-back" id="autocompleteBack">← Voltar</div>';
        artists.forEach(a => {
            const count = allSongs.filter(s => s.artista === a).length;
            html += '<div class="autocomplete-item" data-type="artist" data-value="' + escapeHtml(a) + '">';
            html += escapeHtml(a) + ' <span class="item-artist">(' + count + ')</span></div>';
        });
        html += '</div>';
        autocompleteItems = [{ type: 'back' }, ...artists.map(a => ({ type: 'artist', value: a }))];
    } else {
        // Normal mode: artists + songs
        const matchingArtists = uniqueArtists.filter(a =>
            normalizeText(a).includes(termNorm)
        ).slice(0, 8);

        if (matchingArtists.length > 0) {
            html += '<div class="autocomplete-section">';
            html += '<div class="autocomplete-section-label">Artistas</div>';
        ...
        const matchingSongs = allSongs.filter(s =>
            normalizeText(s.musica).includes(termNorm) &&
            !matchingArtists.includes(s.artista)
        ).slice(0, 8);

        if (matchingSongs.length > 0) {
            html += '<div class="autocomplete-section">';
            html += '<div class="autocomplete-section-label">Músicas</div>';
        ...
        autocompleteItems = [
            ...matchingArtists.map(a => ({ type: 'artist', value: a })),
            ...matchingSongs.map(s => ({ type: 'song', value: s.musica, artist: s.artista }))
        ];
    }
    ...
    autocompleteDropdown.querySelectorAll('.autocomplete-item, .autocomplete-back').forEach(el => {
        el.addEventListener('mousedown', (e) => {
            e.preventDefault();
            if (el.classList.contains('autocomplete-back')) {
                autocompleteLetterMode = false;
            ...
            selectAutocomplete({ type, value, artist, letter });
        });
    });
}
...
function selectAutocomplete(item) {
    if (!item) return;
    if (item.type === 'letter') {
        autocompleteLetterMode = item.letter;
        showAutocomplete(item.letter);
        searchInput.focus();
        return;
    }
    if (item.type === 'artist') {
...
function songMatchesStyles(song, selectedTags) {
    ...
    return selectedTags.some(tag => parseEstilos(song).includes(tag));
}
function songMatchesLangs(song, selectedLangs) {
    if (selectedLangs.length === 0) return true;
    return selectedLangs.some(lang => parseIdiomas(song).includes(lang));
}
function songMatchesDecades(song, selectedDecades) {
    if (selectedDecades.length === 0) return true;
    const decade = getDecade(song.ano);
...
function sortSongs(songs, column, dir) {
    ...
    return [...songs].sort((a, b) => {
        ...
        } else if (column === 'estilo') {
            valA = parseEstilos(a)[0] || '';
            valB = parseEstilos(b)[0] || '';
        } else if (column === 'musica') {
            valA = a.musica; valB = b.musica;
        } else {
            valA = a.artista; valB = b.artista;
        }
...
function renderResults() {
    ...
    html += visible.map(song => {
        ...
        const estilos = parseEstilos(song);
        const tags = estilos.map(e => '<span>' + escapeHtml(e) + '</span>').join('');
    ...
    if (sorted.length > displayLimit) {
        ...
        html += '<div class="show-more" id="showMore">Mostrar mais (' + remaining + ' restantes)</div>';
    }
...
function toggleArtistStar(artist) {
    ...
    songs.forEach(s => {
        songStates[artist + '|' + s.musica] = allStarred ? 'neutral' : 'starred';
    });
    renderArtistList(getFilteredArtists());
    renderResults();
    updateStats();
...
function getFilteredArtists() {
    ...
    return uniqueArtists.filter(a => a.toLowerCase().includes(term));
}
...
function updateStickyButton() {
    if (filteredSongs.length === 0) {
        ...
        stickyDownload.textContent = '📥 Baixar PDF';
        return;
    }
    const starredCount = Object.values(songStates).filter(s => s === 'starred').length;
    if (starredCount > 0) {
        stickyDownload.textContent = '📥 PDF · ' + filteredSongs.length + ' · ' + starredCount + '★';
    } else {
        stickyDownload.textContent = '📥 PDF · ' + filteredSongs.length;
    }
    stickyDownload.disabled = false;
}

function updateFilterToggleBadge() {
    const styleCount = document.querySelectorAll('#styleFilters input:checked').length;
    const langCount = document.querySelectorAll('#langFilters input:checked').length;
    const decadeCount = document.querySelectorAll('#decadeFilters input:checked').length;
    const total = styleCount + langCount + decadeCount;
    if (total > 0) {
        filterToggleBadge.textContent = total;
        filterToggleBadge.classList.add('visible');
    } else {
        filterToggleBadge.classList.remove('visible');
    }
}
...
function generatePDF() {
    ...
    Object.keys(letterGroups).sort().forEach(letter => {
        ...
        sortedArtists.forEach(artist => {
            ...
            const artistSongs = artistGroups[artist].sort((a, b) => a.musica.localeCompare(b.musica, 'pt-BR'));
    ...
    const printHtml =
        ...
        '</style></head><body>' +
        '<div class="header"><h1>REPERTÓRIO</h1>' +
        '<div class="subtitle">BullaAcoustic · Guilherme Bulla — Voz &amp; Violão</div></div>' +
